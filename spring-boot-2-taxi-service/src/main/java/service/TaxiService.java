...
import exception.TaxiIdNotFoundException;
import model.Taxi;
...
public class TaxiService {
    ...
    public Mono<Taxi> updateLocaltion(String taxiId, LocationDTO locationDTO) {
        Optional<Taxi> taxiOptional = taxiRepository.findById(taxiId);
        if (taxiOptional.isPresent()) {
            Taxi taxi = taxiOptional.get();
            return reactiveRedisTemplate.opsForGeo().add(taxi.getTaxiType().toString(), locationToPointConverter.convert(locationDTO), taxiId.toString()).flatMap(
                    l -> Mono.just(taxi)
            );
        } else {
            throw getTaxiIdNotFoundException(taxiId);
